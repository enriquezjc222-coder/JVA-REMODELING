...
function validate(body){
  ...
  const files=Array.isArray(body.files)?body.files:[]; if(files.length>MAX_FILES)throw new Error('Maximum 10 files.');
  let total=0;
  const checked=files.map((f,i)=>{const name=cleanText(f.name,160),type=cleanText(f.type,100),size=Number(f.size||0);if(!ALLOWED_TYPES.has(type))throw new Error(`Unsupported file type: ${name}`);const max=type.startsWith('video/')?MAX_VIDEO:MAX_IMAGE;if(size<=0||size>max)throw new Error(`File too large: ${name}`);total+=size;return {name,type,size,safeName:safeName(name,i)};});
...
exports.createEstimateRequest = onRequest({region:REGION,timeoutSeconds:60,memory:'256MiB'},async(req,res)=>{
  ...
  try{
    const data=validate(req.body||{}), ref=db.collection('estimateRequests').doc(), requestId=ref.id, expiresAt=admin.firestore.Timestamp.fromMillis(Date.now()+SEVEN_DAYS_MS);
    const files=[];
  ...
  }catch(e){console.error(e);res.status(400).json({error:e.message||'Invalid request.'});}
});
...
async function sendEmail(record,downloadUrl){
  ...
  const html=`<h2>New JVA Remodeling estimate request</h2><p><b>Name:</b> ${safe(record.fullName)}</p><p><b>Phone:</b> ${safe(record.phone)}</p><p><b>Email:</b> ${safe(record.email)}</p><p><b>Project:</b> ${safe(record.projectType)}</p><p><b>Description:</b><br>${safe(record.description).replace(/\n/g,'<br>')}</p><p><b>Files:</b> ${(record.files||[]).length}</p>${downloadUrl?`<p><a href="${downloadUrl}">Download project files as ZIP</a> (available for about 7 days)</p>`:'<p>No files were uploaded.</p>'}`;
...
exports.finalizeEstimateRequest = onRequest({region:REGION,timeoutSeconds:540,memory:'1GiB',secrets:[GMAIL_APP_PASSWORD]},async(req,res)=>{
  if(cors(req,res))return; if(req.method!=='POST')return res.status(405).json({error:'Method not allowed.'});
  try{
    const requestId=cleanText(req.body?.requestId,80); if(!/^[A-Za-z0-9_-]{10,}$/.test(requestId))throw new Error('Invalid request ID.');
    const ref=db.collection('estimateRequests').doc(requestId), snap=await ref.get(); if(!snap.exists)throw new Error('Request not found.'); const record=snap.data(); if(record.status==='complete')return res.json({ok:true});
  ...
  }catch(e){console.error(e);res.status(400).json({error:e.message||'Could not finalize request.'});}
});
